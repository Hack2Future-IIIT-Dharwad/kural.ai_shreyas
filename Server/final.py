...
async def convert_audio_to_wav(input_path: str, output_path: str, target_sample_rate: int = 16000):
    try:
        command = [
            "ffmpeg", "-i", input_path,
    ...
    try:
        while True:
            ...
            if data["type"] == "audio":
                ...
                temp_input = f"temp_input_{client_id}.m4a"
                ...
                # success = await convert_audio_to_wav(temp_input, temp_output)
                
                if success:
                    await manager.send_message("Transcribing audio...", client_id)
                    transcription = await transcribe_audio(temp_output)
